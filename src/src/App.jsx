...
import { COEF, TIPOS_OBRA, GRUPOS_OBRA } from './data/coeficientes.js';
...
function estimarMeses(tipoObra, m2) {
  if (tipoObra === 'vivienda_nueva') return Math.max(6, Math.round(m2 / 25));
  if (tipoObra === 'edificio')       return Math.max(12, Math.round(m2 / 20));
  if (tipoObra === 'ampliacion')     return Math.max(3, Math.round(m2 / 30));
  if (tipoObra.startsWith('reforma_todas')) return Math.max(4, Math.round(m2 / 20));
  if (tipoObra.startsWith('reforma_')) return Math.max(1, Math.round(m2 / 15));
  return Math.max(3, Math.round(m2 / 25));
}

// ── Panel lateral: datos de la obra ──────────────────────────
function PanelObra({ datos, onChange }) {
  const mod = MODELOS.find(m => m.tipo === datos.tipoObra);
  const esReforma = datos.tipoObra.startsWith('reforma_');
  const grupoActual = esReforma ? 'reforma' : 'nueva';
  const mesesEstimados = estimarMeses(datos.tipoObra, datos.m2);

  return (
    <div style={S.card}>
      <div style={{ fontWeight:800, fontSize:16, marginBottom:16 }}>📋 Datos de la obra</div>

      {/* Selector de grupo: Obra nueva vs Reforma */}
      {GRUPOS_OBRA.map(grupo => (
        <div key={grupo.id} style={{ marginBottom:10 }}>
          <label style={S.label}>{grupo.label}</label>
          <div style={{ display:'flex', flexWrap:'wrap', gap:4 }}>
            {grupo.tipos.map(t => {
              const activo = datos.tipoObra === t.id;
              return (
                <div key={t.id}
                  style={{
                    ...S.chip(activo, grupo.id === 'reforma' ? '#f59e0b' : '#3b82f6'),
                    fontSize:11,
                    padding:'4px 10px',
                  }}
                  onClick={() => onChange('tipoObra', t.id)}
                >
                  {t.label}
                </div>
              );
      ...
      {mod && <div style={{ fontSize:12, color:'#64748b', marginBottom:12, lineHeight:1.5, background:'#f8fafc', padding:'8px 10px', borderRadius:8 }}>
      ...
      <input style={S.input} type="number" min="1" max="20" value={datos.nBanos} onChange={e=>onChange('nBanos',+e.target.value)} />

      {/* Locales húmedos */}
      <label style={S.label}>Locales húmedos (cocinas)</label>
      <input style={S.input} type="number" min="1" max="10" value={datos.nLocales} onChange={e=>onChange('nLocales',+e.target.value)} />
      ...
      <div style={{ background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:8, padding:'8px 12px', marginBottom:12 }}>
        <div style={{ fontSize:11, color:'#16a34a', fontWeight:700, marginBottom:2 }}>⏱ Duración estimada</div>
        <div style={{ fontSize:16, fontWeight:800, color:'#15803d' }}>{mesesEstimados} meses</div>
        <div style={{ fontSize:10, color:'#4ade80' }}>Calculado según tipo de obra y superficie</div>
      </div>
...
export default function App() {
  const [datos, setDatos] = useState({ tipoObra:'vivienda_nueva', calidad:'estandar', m2:100, nBanos:2, nLocales:1 });
  ...
  const nMesesCalc = estimarMeses(datos.tipoObra, datos.m2);

  const calcular = () => {
    const ls = calcularObra({ tipoObra:datos.tipoObra, m2:datos.m2, nBanos:datos.nBanos, nLocales:datos.nLocales, nMeses:nMesesCalc, calidad:datos.calidad });
    setLineas(ls);
